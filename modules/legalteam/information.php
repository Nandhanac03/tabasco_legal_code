...
if ($_POST) {
    ...
    if (isset($_POST['user_name'])) {
        ...
        $data['user_Id']      =  '';
        $data['user_typeId']  = 23; // Assuming 23 is the user type ID for Leagl staff
        ...
        $data['user_legal_access']  = 'Y'; // Assuming legal access is always 'Y' for legal team members
        $data['user_module']  = 'TL';

        $data['user_loginname'] = $_POST['user_loginname'];
...
if (isset($_FILES['profilePhoto']) && $_FILES['profilePhoto']['error'] === UPLOAD_ERR_OK) {
    ...
    $data['user_profile'] = 'default.png'; // Default profile image
        ...
        if ($ObjUsersClass->processUsers($data)) {
            ...
            $_SESSION['PAGE_SUCCESS'] = 'You have successfully ' . ($edit_id ? 'updated' : 'added') . ' the Legal Team ';
            header("location: " . ROOT_DIR . "legalteam/list.html");
            exit;
        } else {
    ...
    switch($action){
        case 'edit':
            ...
            if ($edit_id > 0)
            $data               =   $ObjUsersClass->get_all_Users($edit_id,'', 23)[0];
            $data['user_photo'] = $data['user_photo'] ? $data['user_photo'] : ''; // Default


            if(!isset($data['user_Id'])){
                header("location: " . ROOT_DIR . "legalteam/information.html");
                exit;
            }
        break;
