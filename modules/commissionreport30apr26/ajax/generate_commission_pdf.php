...
if (empty($rows) || empty($ids)) {
    ...
    exit('No rows selected or IDs missing');
}
...
if (is_array($existing_vouchers) && count($existing_vouchers) === 1) {
    ...
    $voucher_no = $v['voucher_no'] ?? ('VC-' . date('ymd') . rand(100,999));
    $pdf_filename = !empty($v['commission_pdf']) ? $v['commission_pdf'] : 'Commission_Voucher_' . $voucher_no . '.pdf';
    ...
    // ✅ Create new voucher
    $voucher_no = 'VC-' . date('ymd') . '-' . rand(1000,9999);
