...
<td>' . htmlspecialchars(strip_tags(str_replace('&nbsp;', ' ', $client['contact_person'] ?? '-'))) . ' ' .
     htmlspecialchars(strip_tags(str_replace('&nbsp;', ' ', $client['mobile_number'] ?? '-'))) . '</td>
                    ...
                    <td>Open</td>
                    <td>
<a type="button" href="' . ROOT_DIR . 'clientdemo/view/info/' . intval($client['id'] ?? 0) . '.html" class="btn btn-primary" title="View"><i class="fadeIn animated bx bx-arrow-from-left"></i></a>
