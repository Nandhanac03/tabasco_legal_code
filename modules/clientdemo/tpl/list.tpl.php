          ...
          <div class="modal-content">
            <div class="modal-header">
              ...
              <button type="button" class="btn btn-sm btn-primary px-5"
                onclick="window.location.href='<?= ROOT_DIR ?>clientdemo/information.html';"><i
...
<script>
  ...
  `);
    ...
    function validateClient(callback) {
      ...
      if (client_id !== '') {
        $.ajax({
          url: '<?= ROOT_DIR ?>modules/clientdemo/ajax/validate_client.php',
          type: 'POST',
          data: {
            client_id: client_id,
            action: 'client_exist'
        ...
        $.ajax({
          ...
          success: function(response) {
            if (response.status === "success" && response.id > 0) {
              window.location.href = "<?= ROOT_DIR ?>clientdemo/information/edit/" + response.id + ".html";
            } else {
...
$.ajax({
  url: "<?= ROOT_DIR ?>modules/clientdemo/ajax/load_client.php",
  type: "POST",
  cache: false,
  data: {
    client: client,
