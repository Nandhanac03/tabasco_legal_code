...
$countResult = $objActiveLegal->Get_LEGAL_TOTAL_COUNT('', '', 'A', '', $search_filter);
    ...
    foreach ($legalData as $row) {
        ...
        echo '<tr>
         ...
         <td>' . htmlspecialchars($row['code'] ?? '-') . '</td>
           ...
           <td style="max-width:280px; min-width:240px; white-space:normal; word-wrap:break-word; overflow-wrap:break-word; text-align: justify;">'
            . htmlspecialchars($row['Present_Legal_Firm_Name'] ?? '-') . '</td>';
        if ($row['legal_status']) {
            if ($row['legal_status'] == 'Active') {
                $case_status = 'OPEN';
            } else {
                $case_status = 'CLOSED';
            }
        }
        echo '<td>' . htmlspecialchars($case_status) . '</td>';
        if ($row['legal_status']) {
       ...
       ' . htmlspecialchars($status) . '
        ...
        } else if ($row['legal_status'] == 'Closed') {
            echo ' <a type="button" class="btn btn-success" href="' . ROOT_DIR . 'closedlegal/view/view/' . intval($row['id'] ?? 0) . '.html' . '"><i class="lni lni-eye"></i></a>';
        } else if ($row['legal_status'] == 'Bad_debts') {
            echo ' <a type="button" class="btn btn-success" href="' . ROOT_DIR . 'baddebts/view/view/' . intval($row['id'] ?? 0) . '.html' . '"><i class="lni lni-eye"></i></a>';
        }
        echo '  </td>
