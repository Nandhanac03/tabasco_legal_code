...
include_once("../../../lib/class/class.legal_action_subcategory.php");
include_once("../../../lib/auth_ajax.php");

$objSubCategory =   new ActionSubcategory();
...
switch ($action) {
    ...
    case 'checkDuplicate':
        ...
        try {
            ...
            $exist = $objSubCategory->get_subcategory($id, $name, $notId, '', '', '');

            if ($exist) {

                echo json_encode(['success' => false, 'msg' => 'Sub Category already exist.']);

                exit;
            } else {

                echo json_encode(['success' => true, 'msg' => 'Sub Category not exist.']);
    ...
    case 'saveCategory':
        ...
        $categorySaved = $objSubCategory->save_subcategory($data, $id);

        if ($categorySaved) {

            if ($id) {

                $response_xml = feedbackMsg(true, 'SUb Category updated successfully.');
            } else {

                $response_xml = feedbackMsg(true, 'Sub Category saved successfully.');
            }

            echo json_encode(['success' => true, 'msg' => 'Sub Category saved successfully.', 'response_xml' => $response_xml]);
    ...
    case 'categoryListing':
        ...
        $categories = $objSubCategory->get_subcategory('', '', '', $search, $limit, $offset);

        $category_count = $objSubCategory->get_subcategory('', '', '', $search);
        ...
        if (!empty($categories)) {
            ...
            $list .= "<tr><td colspan='3' style='text-align:center;'>No Sub Category available !</td></tr>";
        }
    ...
    case 'deleteCategory':
        ...
        $areaDisabled = $objSubCategory->disable_subcategory($data, $id);

        if ($areaDisabled) {

            $response_xml = feedbackMsg(true, 'Sub Category deleted successfully.');
