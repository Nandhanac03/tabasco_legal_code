...
switch ($action) {
    ...
    case 'checkDuplicate':
        ...
        try {
            ...
            $exist = $objCategory->get_category($id, $name, $notId);
            ...
            if ($exist) {
                ...
                echo json_encode(['success' => true, 'msg' => 'Category not exist.']);
    ...
    case 'saveCategory':
        ...
        $categorySaved = $objCategory->save_category($data, $id);

        if ($categorySaved) {

            if ($id) {

                $response_xml = feedbackMsg(true, 'Category updated successfully.');

            } else {

                $response_xml = feedbackMsg(true, 'Category saved successfully.');

            }

            echo json_encode(['success' => true, 'msg' => 'Category saved successfully.', 'response_xml' => $response_xml]);
    ...
    case 'categoryListing':
        ...
        $categories = $objCategory->get_category('', '', '', $search, $limit, $offset);

        $category_count = $objCategory->get_category('', '', '', $search);
    ...
    case 'deleteCategory':
        ...
        $areaDisabled = $objCategory->disable_category($data, $id);

        if ($areaDisabled) {

            $response_xml = feedbackMsg(true, 'Category deleted successfully.');
