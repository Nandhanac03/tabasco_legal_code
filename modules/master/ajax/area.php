...
include_once("../../../lib/class/class.legal_area.php");
...
switch ($action) {
    ...
    case 'checkDuplicate':
        ...
        try {
            ...
            $exist = $objArea->Get_Area($id, $title, $notId);

            if ($exist) {

                echo json_encode(['success' => false, 'msg' => 'Area already exist.']);

                exit;
            } else {

                echo json_encode(['success' => true, 'msg' => 'Area not exist.']);
    ...
    case 'saveArea':
        ...
        $areaSaved = $objArea->Save_Area($data, $id);

        if ($areaSaved) {

            if ($id) {

                $response_xml = feedbackMsg(true, 'Area updated successfully.');
            } else {

                $response_xml = feedbackMsg(true, 'Area saved successfully.');
            }

            echo json_encode(['success' => true, 'msg' => 'Area saved successfully.', 'response_xml' => $response_xml]);
    ...
    case 'areaListing':
        ...
        $areas = $objArea->Get_Area('', '', '', $search, $limit, $offset);

        $area_count = $objArea->Get_Area('', '', '', $search);

        $area_count = empty($area_count) ? 0 : count($area_count);
        ...
        if (!empty($areas)) {
                ...
                if (defined('LEGAL_AUTH_EDIT') && LEGAL_AUTH_EDIT) {
                    $list .= "
                                        ...
                                        <button type='button' class='btn btn-warning' title='Edit'
                                            ...
                                            data-bs-toggle='modal' data-bs-target='#editModal' data-area_title='{$area['title']}' data-area_id='{$area['id']}'>
                ...
                if (defined('LEGAL_AUTH_DELETE') && LEGAL_AUTH_DELETE) {
                    $list .= "
                                        ...
                                        <button type='button' class='btn btn-danger' title='Delete'
                                            ...
                                            data-bs-toggle='modal' data-bs-target='#deleteModal' data-area_title='{$area['title']}' data-area_id='{$area['id']}'>
            ...
            $list .= "<tr><td colspan='3' style='text-align:center;'>No Area available !</td></tr>";
        }

        echo json_encode(['success' => true, 'html' => $list, 'area_count' => $area_count]);
    ...
    case 'deleteArea':
        ...
        $areaDisabled = $objArea->Disable_Area($data, $id);

        if ($areaDisabled) {

            $response_xml = feedbackMsg(true, 'Area deleted successfully.');
