...
if ($_POST) {
    ...
    if (isset($_POST['user_name'])) {
        if ($action == 'edit')
            ...
            $data['user_Id']      =  '';
        $data['user_typeId']  = 24; // Assuming 21 is the user type ID for internal staff
        ...
        if ($ObjUsersClass->processUsers($data)) {
            ...
            $_SESSION['PAGE_SUCCESS'] = 'You have successfully ' . ($edit_id ? 'updated' : 'added') . ' ';
            header("location: " . ROOT_DIR . "master/lawyer.html");
            exit;
        } else {
    ...
    switch ($action) {
        case 'edit':
            ...
            if ($edit_id > 0)
                ...
                $dataList = $ObjUsersClass->get_all_Users($edit_id, '', 23);
            if (empty($dataList)) {
                $dataList = $ObjUsersClass->get_all_Users($edit_id, '', 24);
            }
            $data = !empty($dataList) ? $dataList[0] : null;


            $data['user_photo'] = $data['user_photo'] ? $data['user_photo'] : ''; // Default
            if (!isset($data['user_Id'])) {
                header("location: " . ROOT_DIR . "master/lawyer_info.html");
                exit;
            }
            break;
    }
