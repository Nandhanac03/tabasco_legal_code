...
$countResult = $objActiveLegal->Get_LEGAL_TOTAL_COUNT('', '', 'A', 'Bad_debts', $search_filter);
...
if ($totalData > 0 && is_array($legalData)) {
    ...
    foreach ($legalData as $row) {
        ...
        ' . htmlspecialchars('Bad Debts') . '
    </button>
            </td>';
        } else {
            echo '<td>' . htmlspecialchars($row['legal_status'] ?? '-') . '</td>';
        }
        echo '<td>' . htmlspecialchars($row['final_total_outstanding'] ?? '-') . '</td>
            ...
            <td>
                ...
                <div class="btn-group">
                    ...
                    <a type="button" class="btn btn-success" href="' . ROOT_DIR . 'baddebts/view/view/' . intval($row['id'] ?? 0) . '.html' . '"><i class="lni lni-eye"></i></a>
