...
$totalData = (int) ($ObjLegalFirm->Get_Legal_Firm_TOTAL_COUNT('', '', 'A', $keyword)[0]['TOTAL_RECORDS'] ?? 0);
...
$filters['status']  =   'A';
$legalFirm      =   $ObjLegalFirm->getLegalFirmInformation($filters);
...
if ($totalData > 0) {
    ...
    foreach ($legalFirm as $legalRows) {

        $edit_url = ROOT_DIR . "legalfirm/information/edit/" . intval($legalRows['id'] ?? 0) . ".html";
        echo '<tr>
                <td>' . htmlspecialchars(strip_tags($legalRows['code'] ?? '-')) . '</td>
                <td>' . htmlspecialchars(strip_tags(str_replace('&nbsp;', ' ', $legalRows['name'] ?? '-'))) . '</td>
                <td>' . htmlspecialchars(strip_tags(str_replace('&nbsp;', ' ', $legalRows['contact_no'] ?? '-'))) . '</td>
                <td>' . htmlspecialchars(strip_tags(str_replace('&nbsp;', ' ', $legalRows['email'] ?? '-'))) . '</td>
                <td>
    <div class="btn-group">

       <a type="button" href="' . ROOT_DIR . 'legalfirm/view/info/' . intval($legalRows['id'] ?? 0) . '.html" class="btn btn-primary" title="View"><i class="fadeIn animated bx bx-arrow-from-left"></i></a>
