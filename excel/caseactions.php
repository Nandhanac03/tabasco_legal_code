...
$sheet->setTitle('Case Actions');

// Set Headers
$headers = ["Date", "Marketing", "Client", "Case Category", "Present Legal Firm"];
...
if ($legalData) {
    foreach ($legalData as $row) {
        $dataRows[] = [
            ...
            $row['ClientName'] ?? '-',
            ucwords(str_replace('_', ' ', $row['legal_firm_type_name'] ?? '-')),
...
header('Content-Disposition: attachment; filename="case_actions_' . date("Y-m-d_H-i-s") . '.xls"');
