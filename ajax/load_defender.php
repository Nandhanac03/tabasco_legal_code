...
if($alphabet=='list'){
    $array_list =   $ObjContact->get_defender('',$parent_id,$parentType);
    ...
    $id         =   trim($_REQUEST['id']);
    $array_data =   array();
    $array_data =   $ObjContact->get_defender($id)[0];
    if(isset($array_data)){
        $input_data =   array();
        $input_data['id']               =   $id;
        $input_data['status']           =   'D';
        ...
        if($ObjContact->Delete_Defender($id,$input_data)){
            echo json_encode(['success' => true, 'message' => ' defender deleted successfully.']);
        }else{
