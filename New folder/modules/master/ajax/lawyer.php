...
$list1 = $ObjUsersClass->get_all_Users(null, $keyword, 23, $offset, $limit) ?: [];
$list2 = $ObjUsersClass->get_all_Users(null, $keyword, 24, $offset, $limit) ?: [];

$usersList = array_merge($list1, $list2);

$count1 = $ObjUsersClass->Get_Users_TOTAL_COUNT('A', $keyword, 23);
$count1 = $count1[0]['TOTAL_RECORDS'] ?? 0;

$count2 = $ObjUsersClass->Get_Users_TOTAL_COUNT('A', $keyword, 24);
$count2 = $count2[0]['TOTAL_RECORDS'] ?? 0;

$totalData = $count1 + $count2;
...
if ($totalData > 0) {
    ...
    foreach ($usersList as $userRows) {
        ...
        $row  = '<tr>';
        $row .= '<td>' . $rowCnt++ . '</td>';
        $row .= '<td>' . htmlspecialchars(strip_tags($userRows['user_name'] ?? '-')) . '</td>';
        $row .= '<td>' . htmlspecialchars(strip_tags(str_replace('&nbsp;', ' ', $userRows['user_address'] ?? '-'))) . '</td>';
        $row .= '<td>' . htmlspecialchars(strip_tags(str_replace('&nbsp;', ' ', $userRows['user_emailId'] ?? '-'))) . '</td>';
        $row .= '<td>' . htmlspecialchars(strip_tags(str_replace('&nbsp;', ' ', $userRows['user_mob'] ?? '-'))) . '</td>';

        if (defined('LEGAL_AUTH_EDIT') && LEGAL_AUTH_EDIT) {
            $row .= '<td>
                        <a href="' . ROOT_DIR . 'master/lawyer_info/edit/' . intval($userRows['user_Id'] ?? 0) . '.html"
