...
switch ($action) {
    ...
    case 'checkDuplicate':
        ...
        try {
            ...
            $exist = $objCaseMode->get_case_mode($id, $name, $notId);

            if ($exist) {

                echo json_encode(['success' => false, 'msg' => 'Case Mode already exist.']);

                exit;
            } else {

                echo json_encode(['success' => true, 'msg' => 'Case Mode not exist.']);
    ...
    case 'saveCaseMode':
        ...
        $categorySaved = $objCaseMode->save_case_mode($data, $id);

        if ($categorySaved) {

            if ($id) {

                $response_xml = feedbackMsg(true, 'Case Mode updated successfully.');
            } else {

                $response_xml = feedbackMsg(true, 'Case Mode saved successfully.');
            }

            echo json_encode(['success' => true, 'msg' => 'Case Mode saved successfully.', 'response_xml' => $response_xml]);
    ...
    case 'caseModeListing':
        ...
        $case_modes = $objCaseMode->get_case_mode('', '', '', $search, $limit, $offset);

        $case_mode_count = $objCaseMode->get_case_mode('', '', '', $search);

        $case_mode_count = empty($case_mode_count) ? 0 : count($case_mode_count);

        if (!empty($case_modes)) {

            foreach ($case_modes as $key => $case_mode) {
                ...
                if (defined('LEGAL_AUTH_EDIT') && LEGAL_AUTH_EDIT) {
                    $list .= "
                                        ...
                                        <button type='button' class='btn btn-warning' title='Edit'
                                            ...
                                            data-bs-toggle='modal' data-bs-target='#editModal' data-casemode_title='{$case_mode['title']}' data-casemode_id='{$case_mode['id']}'>
                ...
                if (defined('LEGAL_AUTH_DELETE') && LEGAL_AUTH_DELETE) {
                    $list .= "
                                        ...
                                        <button type='button' class='btn btn-danger' title='Delete'
                                            ...
                                            data-bs-toggle='modal' data-bs-target='#deleteModal' data-casemode_title='{$case_mode['title']}' data-casemode_id='{$case_mode['id']}'>
            ...
            $list .= "<tr><td colspan='3' style='text-align:center;'>No Case Mode available !</td></tr>";
        }

        // echo"<pre>";print_r($list);exit;

        echo json_encode(['success' => true, 'html' => $list, 'case_mode_count' => $case_mode_count]);
    ...
    case 'deleteCaseMode':
        ...
        $areaDisabled = $objCaseMode->disable_case_mode($data, $id);

        if ($areaDisabled) {

            $response_xml = feedbackMsg(true, 'Case Mode deleted successfully.');
