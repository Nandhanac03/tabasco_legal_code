...
<script>
    $(document).ready(function() {
                    ...
                    success: function(activeLegalArray) {
                        if (Array.isArray(activeLegalArray) && activeLegalArray.length > 0) {
                            activeLegalArray.forEach(item => {
                                $selectLegal.append(`<option value="${item.id}" data-claim="${item.claim_amount}" data-collected="${item.total_collection}">${item.code}</option>`);
                            });
                        } else {
                    ...
                    success: function(caseArray) {
                        if (Array.isArray(caseArray) && caseArray.length > 0) {
                            caseArray.forEach(item => {
                                $selectCase.append(`<option value="${item.id}" data-claim="${item.claim_amount}" data-collected="${item.total_collection}">${item.case_number}</option>`);
                            });
                        } else {
        ...
        function handleFormSubmission($form, config) {
            ...
            // Validation for Collection: don't exceed claim amount
            if (config.formId === '#collection_modal') {
                const $selectedCase = $form.find('.select-active-legal-case option:selected');
                const $selectedLegal = $form.find('.select-active-legal option:selected');
                
                // Use case data if available, otherwise fallback to legal data
                let claimAmount = 0;
                let collectedAmount = 0;
                
                if ($selectedCase.val()) {
                    claimAmount = parseFloat($selectedCase.data('claim')) || 0;
                    collectedAmount = parseFloat($selectedCase.data('collected')) || 0;
                } else if ($selectedLegal.val()) {
                    claimAmount = parseFloat($selectedLegal.data('claim')) || 0;
                    collectedAmount = parseFloat($selectedLegal.data('collected')) || 0;
                }
                
                const newAmount = parseFloat($form.find('.input-amount').val()) || 0;

                if (claimAmount > 0 && (collectedAmount + newAmount > claimAmount)) {
                    const remaining = claimAmount - collectedAmount;
                    alert('Collection amount exceeds claim amount. Remaining balance: ' + remaining.toFixed(2));
                    return;
                }
            }
