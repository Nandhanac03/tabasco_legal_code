...
if ($_POST) {
    ...
    if ($_POST['dateon'] != '' && $_POST['code'] != '' && $user_id != '' && $client != '' && $_POST['category'] != '' && $_POST['agencies_id'] != '') {
        ...
        $active_legal_id = '';
        if (isset($input_data)) {
            // Validation: Claim amount should not be less than collected amount
            if ((float)$input_data['claim_amount'] < (float)$input_data['collected_amount']) {
                $data['error'] = "Claim amount cannot be less than the amount already collected (" . number_format($input_data['collected_amount'], 2) . ").";
                $data = array_merge($_POST, $data); // Keep form data
            } else if ($objActiveLegal->Manage_ActiveLegal($input_data, $update_id)) {
