    ...
    <div class="page-content">
        ...
        <div class="d-flex justify-content-between">
            <div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
                <div class="breadcrumb-title pe-3">Followup Action Update</div>
                <div class="ps-3">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb mb-0 p-0 align-items-center">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">Followup Case Action </li>
                        </ol>
                    </nav>
                </div>
            </div>
                    ...
                    <div class="card-body">
                        ...
                        <div class="table-responsive mt-3">
                            <table class="table align-middle mb-0">
                                ...
                                <tbody>
                                    <?php
                                    if ($legal_precase || $legal_case) {
                                        if ($legal_precase) { ?>
                                            <?php foreach ($legal_precase as $case) { ?>
                                                <tr>
                                                    <td><?= $case['temp_case_number'] ?></td>
                                                    <td><?= $case['case_mode_title'] ? $case['case_mode_title'] : '..' ?></td>
                                                    <td><?= $case['register_date'] ?></td>
                                                    <td><?= $case['hearing_date'] ? $case['hearing_date'] : '.....' ?></td>
                                                    <td><?= $active_legal[0]['total_outstanding']  ?></td>
                                                    <td>
                                                        <button
                                                            onclick="window.location.href='<?= ROOT_DIR ?>actionreport/followupview/view/<?= $case['id'] ?>.html'"
                                        ...
                                        if ($legal_case) { ?>
                                            <?php foreach ($legal_case as $case) { ?>
                                                <tr>
                                                    ...
                                                    <td>
                                                        <button
                                                            onclick="window.location.href='<?= ROOT_DIR ?>actionreport/followupview/caseview/<?= $case['id'] ?>.html'"
                                                            type="button" class="btn btn-outline-success" title="View Case"><i
                                                                class="lni lni-eye"></i>
                                                        </button>
                                                    </td>
