...
require_once 'class.legal_activity_log.php'; 
...
class Login extends dbcon {
    ...
    function Agencies_Login_Authentication($user_name = '', $user_password = '') {
        ...
        if ($this->_num_rows > 0) {
            ...
            $decryptedPassword = decryptPassword($row->password, secretKey);
            if ($user_password === $decryptedPassword) {
                ...
                $activityLog->logActivity(
                    ...
                    'Agency Login',
                    $row->user_id,
                    "Agency user {$row->user_name} logged in"
                );
