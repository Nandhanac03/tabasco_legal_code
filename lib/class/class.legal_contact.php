...
class Contact extends dbcon
{
    ...
    function manage_contact($data = '', $id = '')
    {
        $params = array();
        $Sqlcmd = "INSERT INTO";
        $Sqlcmd = $Sqlcmd . " legal_contacts SET ";
        ...
        $Sqlcmd = $Sqlcmd . " ,profession=:profession";
    ...
    function get_contact($id = '', $parent_id = '', $parent_type = '')
    {
        ...
        $params = array();
        $Sqlcmd = "SELECT * FROM legal_contacts  WHERE 1";
        if ($id) {
            $Sqlcmd = $Sqlcmd . " AND legal_contacts.id=:id";
            $params['id'] = $id;
        }
        if ($parent_id) {
            $Sqlcmd = $Sqlcmd . " AND legal_contacts.parent_id=:parent_id";
            $params['parent_id'] = $parent_id;
        }
        if ($parent_type) {
            $Sqlcmd = $Sqlcmd . " AND legal_contacts.parent_type=:parent_type";
            $params['parent_type'] = $parent_type;
        }
        $Sqlcmd = $Sqlcmd . " AND legal_contacts.status=:statusIs";
        $params['statusIs'] = 'A';
        //echo $this->get_query($Sqlcmd, $params); exit;
    ...
    function Delete_Contact($id = '', $data = '')
    {
        $params = array();
        $Sqlcmd = "UPDATE";
        $Sqlcmd = $Sqlcmd . " legal_contacts SET ";
