<?php
class Expense extends dbcon
{
    ################	MEMBER FUNCTIONS SECTION	##############
    function save_expense($data = '', $id = '')
    {
        $params = array();
        $fields = array();
    ...
    function getting_expense($id = '', $case_id = '', $type = '')
    {
        $params = array();
        $Sqlcmd = "SELECT * FROM legal_expense WHERE 1";
        if ($id) {
    ...
    function get_expense($id = '', $filters = [])
    {
        $params = [];
        ...
        FROM legal_expense le
        LEFT JOIN legal_client lc ON le.client_id = lc.id
        LEFT JOIN legal_activelegal al ON le.active_legal_id = al.id
        LEFT JOIN users U ON U.user_Id = le.marketing_id
        LEFT JOIN legal_fees_type FT ON FT.id = le.fees_type
    ...
    ";
        ...
        if (!empty($filters['client_id'])) {
            $Sqlcmd .= " AND le.client_id = :client_id";
        ...
        if (!empty($filters['date_from']) && !empty($filters['date_to'])) {
            $Sqlcmd .= " AND le.date BETWEEN :date_from AND :date_to";
            $params['date_from'] = $filters['date_from'];
            $params['date_to']   = $filters['date_to'];
        }

        $Sqlcmd .= " ORDER BY le.id DESC";
    ...
    function total_expense($active_legal_id = '', $case_id = '')
    {
        $params = [];
        $Sqlcmd = "SELECT SUM(amount) AS total_amount FROM legal_expense WHERE status = :status";
        $params['status'] = 'A';
        ...
        // Run the query and fetch result
        $this->_result = $this->SELECT_MultiFetch($Sqlcmd, $params);
    ...
    function delete_hearing($data = '', $id = '')
    {
        $params = array();
        $Sqlcmd = "UPDATE";
        $Sqlcmd = $Sqlcmd . " legal_expense SET ";
