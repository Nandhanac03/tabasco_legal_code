...
<div class="card">
    <div class="card-header">
        <h6 class="mb-0"><i class="fadeIn animated bx bx-plus"></i> Add plantiff</h6>

    </div>
    <div class="card-body">
        <form class="row g-3" action="" id="frm_addPlantiff" name="frm_addPlantiff" method="post"
            enctype="multipart/form-data">

            <div class="col-6">
                <label class="form-label">Name <span class="asterisk text-danger">*</span></label>
                <input type="text" class="form-control" id="plantiffName" name="plantiffName" value=""
                    autocomplete="off" />
            </div>
            <div class="col-6">
                <label class="form-label">Contact Number</label>
                <input type="text" class="form-control" id="plantiffNo" name="plantiffNo" value="" autocomplete="off" />
            </div>

            <div class="col-6">
                <label class="form-label">Email</label>
                <input type="email" class="form-control" id="plantiffEmail" name="plantiffEmail" value=""
                    autocomplete="off" />
            </div>
...
<div class="card">
    <div class="card-header">
        <h6 class="mb-0"><i class="lni lni-menu"></i> View Plantiff</h6>
    ...
    <table class="table align-middle mb-0" id="plantiffTable">
...
<script>
    ...
    function fetchAndPopulateTable(ID,moduleIs,pageIs) {
        const tbody = document.querySelector('#plantiffTable tbody');
        tbody.innerHTML = '';

        const url = `<?= ROOT_DIR ?>ajax/load_plantiff.php?alphabet=list&parent_id=${ID}&list_module=${moduleIs}&list_page=${pageIs}`;
    ...
    function handleDelete(id, row) {
        ...
        fetch(`<?= ROOT_DIR ?>ajax/load_plantiff.php?alphabet=delete&id=${id}&parent_id=${<?= $_GET['param1']; ?>}&list_module=${'<?= $_GET['module']; ?>'}&list_page=${'<?= $_GET['page']; ?>'}`, {
            method: 'POST',
            ...
            .then(({ success, message }) => {
                if (success) {
                    console.log(message);
                    alert('✅ Plantiff deleted successfully.');
...
$(document).ready(function () {
    $("#btnAdd").click(function (e) {
        ...
        $.ajax({
            url: '<?= ROOT_DIR ?>/ajax/add_plantiff.php',
            type: 'POST',
            data: formData,
            contentType: false,
            processData: false,
